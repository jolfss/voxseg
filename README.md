...
5. Now you can find `omni.vseg` extension in the top left search bar. Select and enable it.
...
* Extension name is a folder name in `exts` folder, in this example: `omni.vseg`. 
...
```bash
> app\omni.code.bat --ext-folder exts --enable omni.vseg
```
...
```bash
> app\kit\kit.exe --ext-folder exts --enable omni.vseg
```
...
```bash
> app\kit\kit.exe --ext-folder exts --enable omni.vseg --enable omni.kit.window.extensions
```
...
1. Run: `app\kit\test_ext.bat omni.vseg  --ext-folder exts`
...
1. copy `exts/omni.vseg` to `exts/[new extension name]`
2. rename python module (namespace) in `exts/[new extension name]/omni/vseg` to `exts/[new extension name]/[new python module]`
